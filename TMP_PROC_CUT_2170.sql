CREATE OR REPLACE
PROCEDURE CUADRA.TMP_PROC_CUT_2170
IS
...
FROM
	...
	FROM
		TMP_SER1 I,
		CUT_UIM_RFS A,
		XVTR_SIEBEL_INFO_SERIES_T S
	WHERE
		I.EXTERNALOBJECTID = A.EXTERNALOBJECTID
		...
		FROM
			CUT_2170 A
		WHERE
			A.C_EXTERNALOBJECTID = I.EXTERNALOBJECTID
                                        )
                ) AA,
	TMP_SER1 BB
WHERE
	AA.EXTERNALOBJECTID = BB.EXTERNALOBJECTID
	AND NOT EXISTS (
	SELECT
		1
	FROM
		UIMUSER.BUSINESSINTERACTION@UIMPRD.WORLD BB
	WHERE
		BB.EXTERNALOBJECTID = AA.EXTERNALOBJECTID
	...
	SELECT
		1
	FROM
		SIEBEL.S_ORDER@SBLPRD.WORLD AA,
		SIEBEL.S_ORG_EXT@SBLPRD.WORLD EE
	WHERE
		EE.OU_NUM_1 = BB.RUT
...
SELECT
	...
	C.VALUE || '_US2166'
FROM
	TMP_UIMSINCRM_DBOX A,
	XVTR_SIEBEL_INFO_SERIES_T B,
	CUT_UIM_RFS C
WHERE
	A.COD_SERIE = B.COD_SERIE
...
SYSDATE,
SYSDATE,
'ERROR EN PROCESO TMP_PROC_EQUIPOS_MTA',
NULL,
'TMP_PROC_CUT_2170');
...
LOOP
	FETCH CUR_SERIES_DELETE
            INTO
	CUR_EXTERNALOBJECTID,
	CUR_COD_SERIE,
	CUR_CHAROWNER_RFS,
	CUR_CONTROL;

EXIT
WHEN CUR_SERIES_DELETE%NOTFOUND;
BEGIN
	CNT_REGS := CNT_REGS + 1;

IF CNT_REGS < CNT_LIM_REGS THEN
BEGIN
BEGIN
		SELECT
	PD2.CHAROWNER
	...
	CUR_CHAROWNER_PHD
FROM
	UIMUSER.PHYSICALDEVICE_CHAR@UIMPRD.WORLD PD2,
...
FROM
	...
	SELECT
		UNIQUE A.EXTERNALOBJECTID,
		TMP_SEARCH_PHYSDEV(CUR_COD_SERIE,
		'CPEBrand') COD_MARCA,
		TMP_SEARCH_PHYSDEV(CUR_COD_SERIE,
		'CPEModel') COD_MODELO,
		TMP_SEARCH_PHYSDEV(CUR_COD_SERIE,
		'CPEMACAddress') COD_MACADDR,
		I.COD_SERIE,
		TMP_SEARCH_PHYSDEV(CUR_COD_SERIE,
		'CPEUNITMACAddress') COD_UNITAD,
		TMP_SEARCH_PHYSDEV(CUR_COD_SERIE,
		'CPETechnology') COD_TECNO,
		TMP_SEARCH_PHYSDEV(CUR_COD_SERIE,
		'CPEType') COD_TYPE
	FROM
		TMP_UIMSINCRM_DBOX I,
		CUT_UIM_RFS A
	WHERE
		I.EXTERNALOBJECTID = CUR_EXTERNALOBJECTID
...
FROM
	...
	SELECT
		UNIQUE I.INT_PP,
		TMP_SEARCH_PHYSDEV_UNASSIGNED(CUR_COD_SERIE,
		'CPEBrand') COD_MARCA,
		TMP_SEARCH_PHYSDEV_UNASSIGNED(CUR_COD_SERIE,
		'CPEModel') COD_MODELO,
		TMP_SEARCH_PHYSDEV_UNASSIGNED(CUR_COD_SERIE,
...
EXIT
WHEN CUR_SN_US2166_US2170%NOTFOUND;
...
SET
	EST_PROC = 'ERROR',
	FEC_FINAL = SYSDATE,
	DES_PROC = GLS_ERROR || '  ***** TMP_PROC_CUT_2170 *****'
WHERE
	ID_CORRELATIVO = NUM_CORR;
